<template>
  ...
  <main class="container">
    ...
    <v-container>
      <v-row>
        <v-col v-for="mealId in userViewHistory" :key="mealId" cols="12" md="4">
          <v-card @click="fetchMealDetails(mealId)">
            <v-img :src="getMealThumb(mealId)" :alt="getMealName(mealId)" height="200px"></v-img>
            <v-card-title>
              <v-btn icon @click.stop="toggleFavorite(mealId)" v-if="isUserLoggedIn">
                <v-icon>
